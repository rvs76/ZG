...
Friend Module Utilitairesmiseajour
    ...
    Public Function DownloadFile(ByVal remoteFilePath As String, ByVal saveFilePath As String, ByVal timeOut As Integer, _
        ...
        Select Case MajAutoFlag
            Case False
                ...
                If _
                    ...
                    My.Settings.BddExists AndAlso My.Settings.DataBaseExpired = False AndAlso Mainform.Handle <> IntPtr.Zero AndAlso TaskbarManager.IsPlatformSupported Then
            ...
            Case True
                ...
                Mainform.Handle <> IntPtr.Zero AndAlso TaskbarManager.IsPlatformSupported Then
                    Dim zGuideTvTaskbar As TaskbarManager = TaskbarManager.Instance
                    zGuideTvTaskbar.SetProgressState(TaskbarProgressBarState.Normal)
                End If
        End Select
        ...
        Try
            ...
            Try
                ...
                _poid = Math.Max(CInt(wresp.ContentLength), 0)
            ...
            Catch ex As Exception
                ...
                If bMessage Then
                    'Permet d'etre toujours devant
                    ' ReSharper disable NotAccessedVariable
                    Dim boxError As DialogResult
                    ' ReSharper restore NotAccessedVariable
                    boxError = MessageBox.Show _
                        (New Form With {.TopMost = True}, ex.Message & Chr(13), _
    ...
    Private Sub ParseXmlPrograms(ByVal pathXml As String)
        ...
        Try
            ...
            Dim xmlTvDocProg As New XmlDocument()
            ...
            Dim memoChaine As String = String.Empty
            ...
            '
            'rvs75 22/07/2015 : optimisation (à l'arrache)
            'évite de rouvrir le même fichier XMLTV alors qu'il a été ouvert pour les chaines
            'jusqu'à 20% si gros fichier et disque dur mecanique (non ssd)
            If Miseajour.xmlTvDoc IsNot Nothing Then
                xmlTvDocProg = Miseajour.xmlTvDoc
            Else
                xmlTvDocProg.Load(pathXml)
            End If


            elementProgram = xmlTvDocProg.DocumentElement.GetElementsByTagName("programme")
            ...
            For Each noeud In elementProgram
                Try
                            ...
                            Select Case .ItemOf(attribCount).Name
                                ...
                                Case "stop"
                                    PStopA = TextAttrib.Substring(0, 4) & "-" & TextAttrib.Substring(4, 2) & "-" & _
                                              TextAttrib.Substring(6, 2) & " " & TextAttrib.Substring(8, 2) & ":" & _
                                              TextAttrib.Substring(10, 2)
                                    'rvs75 22/07/2015 : optimisation
                                    'si la date est dépassé, ça sert à rien de continuer
                                    If Date.Compare(CDate(PStopA), dateFin) < 0 Then Exit For
                                Case "showview"
                                ...
                                Case "channel"
                                    ...
                                    'Next item
                                    'rvs75 22/07/2015 : optimisation
                                    'au lieu de boucler à chaque programmes, le bouclage se fait si la chaine du programme précédent est différent de la chaine du programme en cours de traitement
                                    'permet de gagner environ 10% de temps si les programmes sont triés par chaines (Kevin et Kazer)
                                    If PChannelA = memoChaine Then
                                        channelFound = True
                                        PIndex += 1
                                    Else
                                        For Each item As String() In Miseajour.CollectionSelectChannels
                                        ...
                                        Next item
                                    End If
                                    'rvs75 22/07/2015 : Ne sont jamais utilisé
                                    'Case "clumpidx"
                    ...
                    'Dim dateFin As Date = Date.Now.AddHours(-6)
                    If channelFound AndAlso Not bperim Then
                                    ...
                                    For Each noeudEnfCredits In noeudEnfProgram
                                        Select Case noeudEnfCredits.Name
                                            Case "director"
                                                ...
                                                PDirector = String.Concat(PDirector, ", ", pTemp)
                                            Case "actor"
                                            ...
                                            Case "writer"
                                                ...
                                                pTemp = noeudEnfCredits.InnerText
                                                PWriter = String.Concat(PWriter, ", ", pTemp)
                                            Case "adapter"
                                                'PAdapter = PAdapter & noeudEnfCredits.InnerText & ", "
                                                pTemp = noeudEnfCredits.InnerText
                                                PAdapter = String.Concat(PAdapter, ", ", pTemp)
                                            Case "producer"
                                            Case "presenter"
                                                'PPresent = PPresent & noeudEnfCredits.InnerText & ", "
                                                pTemp = noeudEnfCredits.InnerText
                                                PPresent = String.Concat(PPresent, ", ", pTemp)
                        ...
                        End If
                        If PDirector.Length > 0 Then
                            PDirector = PDirector.Substring(2)
                        End If
                        If PWriter.Length > 0 Then
                            PWriter = PWriter.Substring(2)
                        End If
                        If PAdapter.Length > 0 Then
                            PAdapter = PAdapter.Substring(2)
                        End If
                        If PPresent.Length > 0 Then
                            PPresent = PPresent.Substring(2)
                        End If
            ...
            ' 26/06/2014 rvs75 : mise à jour des couleurs catégories
            Dim listeCategoriesNouveau As IEnumerable(Of String) = ListeCategoriesNouvellesAprèsMaj()
            If listeCategoriesNouveau.Count > 0 Then
                AddCategorie(listeCategoriesNouveau)
            End If

            '  Néo le 14/07/2014
            ' On affiche pas le message au premier lancement de zg
            If listeCategoriesNouveau.Count > 0 AndAlso My.Settings.GestionCategorie = False Then
                'Permet d'etre toujours devant
                If MessageBox.Show(New Form With {.TopMost = True}, MessageNouvellesCategories1 & Environment.NewLine & MessageNouvellesCategories2,
                                    MessageNouvellesCategoriesTitre, MessageBoxButtons.YesNo) = DialogResult.Yes Then
                    ''afficher la fenetre de gestion des catégories
                    GestionCategorie.ShowDialog()
                End If
            End If

            ' Néo le 14/07/2014
            ' Au premier lancement de zg on n'affiche pas le message ci-dessus. On passe le flag à False pour les mises à jour suivantes.
            My.Settings.GestionCategorie = False
